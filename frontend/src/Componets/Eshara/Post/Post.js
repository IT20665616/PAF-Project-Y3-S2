...
function Post() {
  ...
  const [userName, setUserName] = useState("Eshara");
  ...
  const onPostClick = () => {
    ...
    if (newComment) {
      ...
      axios
        .post("http://localhost:8090/addComment", data, {
          headers: {
  ...
  const onDeleteClick = (id) => {
    axios.delete(`http://localhost:8090/deleteComment/${id}`)
    .then(response => {
  ...
  const onUpdateClick = () => {
    axios
      .put("http://localhost:8090/editComment", {
        id: updateId,
        user_name: userName,
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8090/getComments/${postId}`)
      .then((res) => {
